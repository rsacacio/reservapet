(function () {
    ...
    angular.module('reservapetApp').factory('AuthenticationFactory', AuthenticationFactory);
    angular.$inject = ['ReservapetHttp', '$http', '$location', 'redirectToUrlAfterLogin', '$window', 'GlobalHolder', 'AuthenticationResourceFactory'];

    function AuthenticationFactory(ReservapetHttp, $http, $location, redirectToUrlAfterLogin, $window, GlobalHolder, AuthenticationResourceFactory) {
        var service = {};
        ...
        service.login = function (username, password, successCallback, errorCallback) {
            ...
            ReservapetHttp.post("api/login", credentials).success(function (response) {
        ...
        service.impersonate = function (username, password, ambianceId) {
            var credentials = {username: username, password: password};
            ReservapetHttp.post("api/impersonation/impersonate", credentials).success(function (response) {
