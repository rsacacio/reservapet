...
var module = angular.module('reservapetApp', [ 'ui.router', 'ngSanitize', 'ngCookies', 'ngAnimate', 'angularSpinners', 'pascalprecht.translate', 'angularModalService', 'angularScreenfull', 'oitozero.ngSweetAlert', 'ui.select', 'daterangepicker', 'ui.sortable', 'toastr', 'summernote']);

module.value('redirectToUrlAfterLogin', {url: '#/dashboard'});
...
module.run(function($rootScope, $location, GlobalHolder, $http, AuthenticationFactory, $document, $state, $translate) {
    var routeChangeStart = $rootScope.$on('$locationChangeSuccess', function(evt) {
        ...
        } else if($location.$$path.indexOf('private') >= 0){
